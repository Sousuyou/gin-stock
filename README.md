...
3. オーナーがSupabaseで内容を確認 → 良い行だけ `status='approved'` に変更（NGは `rejected`）
4. `python3 promote_pending.py` 実行 → approved 行を `gins.json` に追記し、`count`/`version` 更新
5. 差分を確認して push（CSVが必要なら `python3 export_csv.py`）
